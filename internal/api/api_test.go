...
func newTestServer(h channel.MessageHandler) *Server {
	return New(":0", h, nil, nil, nil, nil, nil, nil)
}
...
func TestAuthMiddleware_MissingToken(t *testing.T) {
	s := New(":0", okHandler, &mockUserLookup{err: errors.New("no user")}, nil, nil, nil, nil, nil)
	w := httptest.NewRecorder()
...
func TestAuthMiddleware_InvalidToken(t *testing.T) {
	s := New(":0", okHandler, &mockUserLookup{err: errors.New("not found")}, nil, nil, nil, nil, nil)
	w := httptest.NewRecorder()
	r := httptest.NewRequest(http.MethodPost, "/api/chat", strings.NewReader(`{"text":"hi"}`))
...
func TestAuthMiddleware_ValidToken(t *testing.T) {
	...
	s := New(":0", okHandler, &mockUserLookup{user: &sqlcgen.User{ID: uid, ApiKeyHash: hash}}, nil, nil, nil, nil, nil)
	w := httptest.NewRecorder()
	r := httptest.NewRequest(http.MethodPost, "/api/chat", strings.NewReader(`{"text":"hi"}`))
